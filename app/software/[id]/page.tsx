...
import { Check, X, Star, Info, Tablet, ExternalLink, ArrowRight, Trash2, BarChart3, HardHat, ChevronRight } from 'lucide-react';
...
export default function Home() {
  ...
  return (
    // FIX SCROLL: max-w-[100vw] e overflow-hidden impediscono alla pagina intera di slittare
    <div className="min-h-screen bg-gray-50 font-sans text-slate-900 flex flex-col w-full max-w-[100vw] overflow-x-hidden">
      ...
      {/* MAIN CONTENT */}
      <main className="max-w-7xl mx-auto px-2 md:px-4 py-8 flex-grow w-full">
        
        {!showDeepCompare ? (
          /* --- VISTA TABELLA CLASSICA --- */
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative">
            
            {/* UX MOBILE: OMBREGGIATURA "FADE" A DESTRA */}
            <div className="absolute top-0 bottom-0 right-0 w-6 bg-gradient-to-l from-slate-200/50 to-transparent pointer-events-none z-10 md:hidden"></div>

            {/* FIX SCROLL: overflow-x-auto QUI permette lo scroll solo della tabella */}
            <div className="w-full overflow-x-auto pb-1">
              <table className="w-full text-left border-collapse min-w-[1000px] divide-x divide-gray-100">
                ...
                <tbody className="divide-y divide-gray-100">
                  ...
                  {/* --- RIGA SPECIALE UX MOBILE (Esce solo su mobile) --- */}
                  <tr className="md:hidden bg-blue-50/50 border-b border-blue-100">
                    <td className="sticky left-0 bg-blue-50/50 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]"></td>
                    <td colSpan={5} className="p-2 text-xs text-blue-700 font-bold flex items-center gap-2 animate-pulse">
                       👉 Scorri la tabella verso destra <ChevronRight size={14} />
                    </td>
                  </tr>
            ...
            <div className="hidden md:flex p-3 bg-blue-50 text-blue-800 text-xs text-center border-t border-blue-100 justify-center items-center gap-2">
               <Info size={14} /> Spunta le caselle a sinistra (massimo 3) e clicca su "Confronta Selezionati".
            </div>
          </div>
        ) : (
          /* --- VISTA DEEP COMPARE --- */
      ...
      {selectedIds.length > 0 && !showDeepCompare && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-6 z-50 animate-in slide-in-from-bottom-10 border border-slate-700 w-[90%] md:w-auto justify-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            ...
            <div className="md:text-right">
              ...
              <ul className="space-y-2">
                <li><Link href="/privacy" className="hover:text-white transition-colors underline decoration-slate-700 underline-offset-4">Privacy Policy</Link></li>
                <li><Link href="/privacy" className="hover:text-white transition-colors underline decoration-slate-700 underline-offset-4">Cookie Policy</Link></li>
              </ul>
            </div>
          </div>
